...
### Fixed
- **Native speaker-route override** (`modules/audio-route`, needs a dev-client rebuild): when iOS still routes PlayAndRecord output to the earpiece, `overrideOutputAudioPort(.speaker)` forces the loudspeaker — Bluetooth/wired routes stay respected. Dev builds also log every audio decision (`[voice]` lines in Metro). Suspected voice interruptions no longer feed the pre-roll (full-volume coach echo) to the transcriber, which was how phantom barge-ins got "confirmed" and killed replies.
- **Replies played into the phone-call earpiece while the mic was hot** (continuous mode never releases the recorder, and its internal session resets dropped iOS's speaker routing) — the playback route is now re-asserted when the recorder starts and every time the coach starts speaking. Suspected-interruption ducking is also time-bounded (2.5 s without transcribed words restores full volume and discards the phantom turn).
